...
function SessionFilledApp(props: {
  darkMode: DarkMode;
  mounted: boolean;
  children: ReactNode;
}) {
...
function InternalKbarUpdater(props: {
  darkMode: DarkMode;
  children: ReactNode;
}) {
  const { status } = useSession();
  const { editing } = useEditing();
  useRegisterActions(
    kbarActions(status, editing, props.darkMode.value, props.darkMode.toggle),
    [status, editing, props.darkMode.value, props.darkMode.toggle]
  );
