...
export const TextField = (props: TextFieldProps) => {
  ...
  useEffect(() => {
    if (textArea.current && props.textLike && spanRef.current) {
      ...
      textArea.current.style.width = `calc(${spanRef.current.scrollWidth}px + ${textSizeFromVariant(props.variant || "p")})`;
    }
  });
...
const Field = styled.textarea<{ error?: string, textLike?: boolean, value?: string, variant?: VariantType, editable?: boolean }>`
  margin: ${({ textLike }) => textLike ? "0" : "15px"};
  min-width: ${({ textLike }) => textLike ? "0px" : "20rem"};
  font-size: ${({ textLike, variant }) => variant ? textSizeFromVariant(variant) : textLike ? "unset" : "1.25rem"};
  ...
  resize: none;
  color: ${({ textLike, theme, editable }) => textLike && editable ? theme.primary : theme.text};
  ::placeholder {
...
const TextLikeHidden = styled.span<{ error?: string, textLike?: boolean, value?: string, variant?: VariantType }>`
  opacity: 0;
  position: absolute;
  pointer-events: none;
  margin: ${({ textLike }) => textLike ? "0" : "15px"};
